...
Button(ventana, text="Seleccionar", command=agregar_directorio).grid(row=0, column=2)

Label(ventana, text="Expresión Regular (buscar):").grid(row=1, column=0, sticky="w")
entrada_patron = Entry(ventana, width=50)
entrada_patron.grid(row=1, column=1, columnspan=2, sticky="w")

Label(ventana, text="Texto de Reemplazo:").grid(row=2, column=0, sticky="w")
entrada_reemplazo = Entry(ventana, width=50)
entrada_reemplazo.grid(row=2, column=1, columnspan=2, sticky="w")

Label(ventana, text="Formatos a incluir (ej.: md, txt):").grid(row=3, column=0, sticky="w")
entrada_incluir = Entry(ventana, width=50)
entrada_incluir.grid(row=3, column=1, columnspan=2, sticky="w")

Label(ventana, text="Formatos a excluir (ej.: log, bak):").grid(row=4, column=0, sticky="w")
entrada_excluir = Entry(ventana, width=50)
entrada_excluir.grid(row=4, column=1, columnspan=2, sticky="w")

backup_var = IntVar()
Checkbutton(ventana, text="Crear respaldo antes de reemplazar", variable=backup_var).grid(row=5, column=1, sticky="w")

Button(ventana, text="Ejecutar", command=ejecutar_reemplazo).grid(row=6, column=1, pady=10)
Button(ventana, text="Ayuda", command=mostrar_ayuda).grid(row=6, column=2, pady=10)
